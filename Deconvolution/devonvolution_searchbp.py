...
class MoleculeData:
    ...
    def readHeader(self):
        ...
        self.path = Path(file)
        self.prefix = self.path.stem + f'_bp{bpConversion*1000:.0f}'
    ...
    def precondition(self): # Preconditioning: iteratively close in on apparent energylandscape to create good parameter guesses        
        ...
        for pos in self.rloopBpPos:
            ...
            found = np.interp(pos, self.bins, np.arange(len(self.bins)))
        ...
        for i in range(10000):
            ...
            result = (self.pApparent - self.guessGaussSum)[self.bpPositionBins[0]-1:self.bpPositionBins[-1]+1]    # difference between current guess and apparent energy landscape, beginning and tail are truncated
    ...
    def setWeights(self):
        ...
        ZeroPos = np.argmax(self.pvalues[0:self.bpPositionBins[6]])+1                    # baseline
        MaxPos = np.argmax(self.pvalues[self.bpPositionBins[23]:-1])+self.bpPositionBins[23]+1   # minimum of rloop

        for i, bin in enumerate(self.bins):
            
            w = 1

            if i <= (ZeroPos - 11): # ignore anything before ~ 1.1 rad of the baseline peak
...
def fitCallback(args):
    ...
    if totalIterations%20 != 1: return
    
    RloopAmps = args[Rloop_argIndexStart:]

    mol:MoleculeData    # type hint
    
    print(f'Iteration {totalIterations},  bpConversion: {bpConversion}')
...
RloopTruncationLength = 1                                   # number of end mismatches
...
MismatchPos = -1                                            # use -1 for no mismatch
if MismatchPos != -1: print(f'Applying mismatch at position {MismatchPos}')

iterations = 10 if test else 2000

dE_truncation = 2.29582 # in kT/rad: fixed penalty for end truncation mismatches of RLoop

x0range = 1   # allowed interval size to look for the angle offset

bpConversion = 0.55        # initial value of bpConversion

bpConversionValues = np.arange(0.55, 0.592, 0.005)

rloopFitPositions = RloopMatchingRange + 1  # how many amplitudes to modulate, +1 to include baseline, +2 to include first mismatch

# EntropyScaleValues = [0, 0.5, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000]
EntropyScaleValues = [100]
EntropyScaleValues = [100] if test else EntropyScaleValues

entropyScale = 100 # initial value of the scaling factor for entropy in minimization
...
MaxPosOffset = 21 if RloopMatchingRange <= 28 else 26 
...
# files = [
#     'Z:/GroupMembers/Dominik/Projects/CRISPR-Twister/CRISPR Energy Landscape Deconvolution/Data/20190830.dat', 
#     'Z:/GroupMembers/Dominik/Projects/CRISPR-Twister/CRISPR Energy Landscape Deconvolution/Data/20190830_4.dat', 
#     'Z:/GroupMembers/Dominik/Projects/CRISPR-Twister/CRISPR Energy Landscape Deconvolution/Data/20190830_5.dat',
...
for bpConversion in bpConversionValues:
    ...
    fitOptions = {'maxcor': 4000, 'ftol': 1e-12, 'gtol': 1e-08, 'eps': 1e-08, 'maxfun': 250000, 'maxiter': iterations, 'maxls': 40}

    constructPrefix = f"T1_bp{bpConversion*1000:.0f}"
    ...
    for i, mol in enumerate(moleculeList):
        ...
        Energies[:,i] = -np.log(mol.fullLocalAmplitudes/(np.sum(mol.fullLocalAmplitudes[1:])+globalFitData.fitAmplitudes[0]))

    # Average measurement data

    EData = -np.log(pData)
    pAverage = np.exp(-np.nanmean(EData, axis=1))
    pAverageSum = np.nansum(pAverage)*binsize_bp
    EStd = np.nanstd(EData, axis=1)
    EAverage = -np.log(pAverage/pAverageSum)
    ...
    ax.plot(interpBins, EAverage, '-', color = '#000000')
    ax.fill_between(interpBins, EAverage+EStd, EAverage-EStd, linestyle='--', edgecolor='#222222', facecolor='#21212185')
